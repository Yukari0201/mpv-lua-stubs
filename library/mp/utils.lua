...
-- >
-- > `dirs`  
-- > List directories only, or symlinks to directories. `.` and `..` are
-- > not included.
-- >
-- > `normal`  
...
function utils.readdir(path) end

-- Stats the given path for information and returns a table with the
-- following entries:
-- 
-- > `mode`  
-- > protection bits (on Windows, always 755 (octal) for directories and
-- > 644 (octal) for files)
-- >
-- > `size`  
-- > size in bytes
...
-- booleans `is_file` and `is_dir` are provided as a convenience; they can
...
-- example, if `p2` is an absolute path, `p2` is returned without change.
-- 
---@param p1 any
---@param p2 any
function utils.join_path(p1, p2) end

-- Runs an external process and waits until it exits. Returns process
-- status and the captured output. This is a legacy wrapper around calling
-- the `subprocess` command with `mp.command_native`. It does the following
-- things:
-- 
-- - copy the table `t`
...
-- Returns the C environment as a list of strings. (Do not confuse this
-- with the Lua \"environment\", which is an unrelated concept.)
-- 
function utils.get_env_list() end

-- Parses the given string argument as JSON, and returns it as a Lua table.
-- On error, returns `nil, error`. (Currently, `error` is just a string
...
-- 
-- If the `trail` parameter is `true` (or any value equal to `true`), then
-- trailing non-whitespace text is tolerated by the function, and the
-- trailing text is returned as 3rd return value. (The 3rd return value is
-- always there, but with `trail` set, no error is raised.)
-- 
function mp.get_property_native() end
